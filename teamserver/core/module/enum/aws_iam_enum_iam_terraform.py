...
def exploit(all_sessions, cred_prof, useragent, web_proxies, callstoprofile):
	...
	useragent = "APN/1.0 HashiCorp/1.0 Terraform/1.8.5 (+https://www.terraform.io) terraform-provider-aws/5.57.0 (+https://registry.terraform.io/providers/hashicorp/aws) aws-sdk-go-v2/1.30.1 os/linux lang/go#1.22.4 md/GOOS#linux md/GOARCH#amd64 api/iam#1.30.1"
	...
	useTerraform = False
	if variables['TERRAFORM']['value'].lower() == "true":
		useTerraform = True
	if variables['TERRAFORM']['value'].lower() != "true" and variables['TERRAFORM']['value'].lower() != "false":
		return {"error": "TERRAFORM should be either true or false."}
